...
import cn.sky1998.mango.system.domain.TreeSelect;
import cn.sky1998.mango.system.domain.dto.MetaVo;
import cn.sky1998.mango.system.domain.dto.RouterVo;
...
public class MenuServiceImpl implements IMenuService
{
    ...
    {
        int result = roleMapper.checkMenuExistRole(menuId);
        return result > 0 ? true : false;
    }
    ...
    {
        String routerName = StringUtils.capitalize(menu.getMenuName());
        // 非外链并且是一级目录（类型为目录）
        if (isMenuFrame(menu))
        {
     ...
     * @param userId
     */
    private void tree(List<Menu> components,Long userId) {
        for (Menu model : components) {
            if (!menuMapper.selectChildIsExist(model.getMenuId()))
                continue;
            List<Menu> menus = menuMapper.ChildTree(model.getMenuId(), userId);
